...
Krikkit::Krikkit() :
    node_handle_("~")
{
  ROS_INFO_STREAM("Create Krikkit");
  odometry_seq_count_ = 1;
}

int Krikkit::run()
{
  ROS_INFO_STREAM("Run Krikkit");

  cmd_vel_subscriber_ = node_handle_.subscribe("/cmd_vel", 1, &Krikkit::cmdVelCallback, this);
  ...
  odom_subscriber_ = can_interface_->subscribeToAll(boost::bind(&Krikkit::odometryCallback,this,_1));
...
void Krikkit::cmdVelCallback(const geometry_msgs::TwistConstPtr & cmd_vel)
{
  ...
  uint8_t dr_raw[2];
  utils::Conversion::uint16Touint8(dr, dr_raw);
  uint8_t ds_raw[2];
  utils::Conversion::uint16Touint8(ds, ds_raw);
  uint8_t dp_raw[2];
  utils::Conversion::uint16Touint8(dp, dp_raw);
...
void Krikkit::odometryCallback(const tug_can_msgs::CanMessageConstPtr & msg)
{
  ...
  uint16_t ds;
  uint16_t dp;
  utils::Conversion::uint8Touint16(dr_data,dr);
  utils::Conversion::uint8Touint16(ds_data,ds);
  utils::Conversion::uint8Touint16(dp_data,dp);
...
void Krikkit::enableOdometrySensor()
{
...
uint32_t Krikkit::getMessageHeader(uint32_t device_id, uint32_t message_id)
{
...
{
  ros::init(argc, argv, "krikkit_base");
  tug_krikkit::Krikkit node;
  return node.run();
}
